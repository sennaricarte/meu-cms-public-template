...
const POST = async ({ request, cookies }) => {
  ...
  const titleHint = typeof titleField === "string" && titleField.trim().length > 0 ? titleField.trim() : "";
  const mime = file.type.trim().toLowerCase();
  if (!mime || !ALLOWED_TYPES.has(mime)) {
    return json(
      {
        error: mime ? `Tipo não permitido: ${mime}. Envie apenas JPEG, PNG ou WebP.` : "Tipo de arquivo ausente ou inválido. Envie apenas JPEG, PNG ou WebP."
      },
      415
    );
  }
